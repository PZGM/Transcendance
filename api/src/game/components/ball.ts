...
export class Ball implements BallDto { //extends?
	...
	constructor(difficulty : Difficulty) {
		...
		this.coor.speed = this.coor.setting.speed / 2;
		this.coor.dx = this.coor.speed * (Math.random() < 0.5 ? 1 : -1);
		this.r = this.coor.setting.width / (3 * (this.coor.difficulty + 3));
		this.goal = 0;
	}
	...
	reset(): void {
		...
		this.coor.speed = this.coor.setting.speed / 2;
		this.coor.dy = 0;
        this.coor.dx = this.coor.speed * ((this.coor.dx < 0) ? -1 : 1);
	}
	...
	playerCollision(time: number, p1: Player, p2: Player): boolean {
		...
		if (this.coor.x < screenSizeX / 2) {
			// p1.width -> 15
			if (((this.coor.x + (this.coor.dx * time)) - this.r) < p1.coor.x + 15)
			{
				...
				{
					// p1.width -> 15
					this.coor.x = (p1.coor.x + 15) + this.r;
