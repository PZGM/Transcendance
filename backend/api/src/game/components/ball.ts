...
export class Ball implements BallDto { //extends?
	...
	constructor(ballDTO: BallDto) {
        this.goal = 0;
		this.coor.setting = new Setting(ballDTO.coor.difficulty);
        ...
        this.coor.dx = ballDTO.coor.setting.speed * (Math.random() < 0.5 ? 1 : -1);
        this.coor.dy = 0;
	...
	update(time: number, p1: Player, p2: Player): void {
        ...
        if(this.coor.x + this.coor.setting.r >= this.coor.screenSizeX)
        {
            p1.goal++;
            this.goal = 1;
        }
	...
	collision(time: number, p1: Player, p2: Player) : boolean {
		if((this.coor.y + (this.coor.dy * time)) - this.coor.setting.r <= 0 || //border Collision
            (this.coor.y + (this.coor.dy * time)) + this.coor.setting.r >= this.coor.screenSizeY)
